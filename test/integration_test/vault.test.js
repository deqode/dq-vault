...
describe("Integration test for generating bitcoin signature", () => {
  ...
  before(done => {
    const data = {
      ...
      passphrase: "",
      mnemonic:
        "similar supreme jealous custom please fitness mosquito report movie valley hip hub slush foam deer"
    };
  ...
  it("should return a valid signature when valid uuid, path, payload and cointype are provided", done => {
    const payload = {
      inputs: [
        {
          txhash:
            "81b4c832d70cb56ff957589752eb412a4cab78a25a8fc52d6a09e5bd4404d48a",
          vout: 0
        }
      ],
      outputs: [
  ...
  it("should return error when invalid payee address is provided", done => {
    ...
    request.post(
      ...
      function(err, response, body) {
        let vaultResponse = JSON.parse(body);
        expect(response.statusCode).to.not.equal(200);
        expect(vaultResponse.errors[0]).to.include(
...
describe("Integration test for generating ethereum signature", () => {
  ...
  it("should return error when gasPrice parameter is negative", done => {
    const payload = {
      nonce: 0,
      value: 100000000000000000,
      gasLimit: 21000,
  ...
  it("should return error when gasLimit parameter is negative", done => {
    const payload = {
      nonce: 0,
      value: 100000000000000000,
